...
export function LegalPage({ title, intro, sections, updated = "27 de julho de 2026" }: { title: string; intro: string; sections: LegalSection[]; updated?: string }) {
  return <>
    <SiteHeader />
    <main className="legal-main">
      <div className="container legal-wrap">
        ...
        <article className="legal-document">
          <div className="legal-head"><span className="eyebrow"><i /> Transparência Layvo</span><h1>{title}</h1><p>{intro}</p><small>Última atualização: {updated}</small></div>
